    ...
    $.each other_data, (i, other_item) ->
      ...
      if link_regex.test(other_item['code'])
        other_html = '<a href="' + other_item['code'] +
          ...
          '<h4 class="list-group-item-heading">' + other_item['desc'] +
        ...
        other_html = '<a data-code="' + encodeURIComponent(other_item['code']) +
          ...
          other_item['desc'] + '</h4>' +
          '<p class="list-group-item-text text-muted"><strong>' +
          other_item['code'] + '</strong></p>'
      other_html += '<p class="list-group-item-text text-muted">' +
        other_item['money'] + '&nbsp;|&nbsp;Timeout:' +
        other_item['time'] + '</p></a>'
