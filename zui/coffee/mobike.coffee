    ...
    $.each mobike_data, (i, mobike_item) ->
      ...
      if link_regex.test(mobike_item['code'])
        mobike_html = '<a href="' + mobike_item['code'] +
          ...
          '<h4 class="list-group-item-heading">' + mobike_item['desc'] +
          '</h4><p class="list-group-item-text text-muted">' +
          'This is a link,click to browse</p>'
      else
        mobike_html = '<a data-code="' +
          encodeURIComponent(mobike_item['code']) +
          '" href="javascript:void(0);" target="_blank" ' +
          'class="list-group-item showCode">' +
          '<h4 class="list-group-item-heading">' + mobike_item['desc'] +
          '</h4>' + '<p class="list-group-item-text text-muted"><strong>' +
          mobike_item['code'] + '</strong></p>'
      mobike_html += '<p class="list-group-item-text text-muted">Money:' +
        mobike_item['money'] + '&nbsp;|&nbsp;Timeout:' +
        mobike_item['time'] + '</p></a>'
...
mobike_showCode = (code) ->
  ...
  if code.length > 0
    ...
    modal_html = '<h1><font color="#FF0000">Web script error (Error code).' +
      '</font></h1>'
